...
MainComponent::MainComponent()
{
    ...
    setLookAndFeel (&CustomLookAndFeel::instance());
    ...
    // --- effect rack ---
    rack.onAddPluginClicked = [this] { openPluginBrowser(); };
...
void MainComponent::updateNowPlaying()
{
    if (! engine.hasTrack())
    {
        trackName.setText ("鏈姞杞芥洸鐩?, juce::dontSendNotification);
        trackMeta.setText ("鎵撳紑鏂囦欢銆佹嫋鍏ラ煶棰戯紝鎴栧弻鍑绘挱鏀惧垪琛ㄤ腑鐨勬洸鐩?, juce::dontSendNotification);
    ...
    trackMeta.setText (juce::String::formatted ("%.1f kHz 路 %d:%02d 路 %s",
